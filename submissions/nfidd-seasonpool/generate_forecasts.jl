...
#
# Submitted as `nfidd-seasonpool` (round-2 seasonal pick; see
# submissions/seabbs_bot-seasonpool/generate_forecasts.jl for the
# identical model under the seabbs_bot team abbreviation).
#
...
const MODEL_ID = "nfidd-seasonpool"
...
"""
    load_valid_origin_dates(hub_path) -> Set{Date}

Every `origin_date` the hub's own `hub-config/tasks.json` actually
lists as a valid round (`rounds[1].model_tasks[1].task_ids.origin_date
.optional`). This repo's own `training_splits` runs a few weeks past
the hub's round list at the tail of season 5 (`data/
flu_data_hhs_tscv_season5.csv` runs to 2020-05-02; the hub's last
round is 2020-03-21, presumably where the COVID-19 disruption to
ILINet made later weeks unusable) -- a submission with `origin_date`
values outside the hub's round list fails `scripts/
validate_submission.jl`'s `check_origin_date`.

Rather than add a JSON dependency to this light/analytic driver, this
locates the `"origin_date": {...}` task_id block by its (colon-
qualified, so it doesn't match the earlier `"round_id": "origin_date"`
string value) key, then regex-matches `\\d{4}-\\d{2}-\\d{2}` only
inside that block's first `[...]` (its `optional` array). Scoping to
that one array specifically matters: `tasks.json` also lists a
`target_end_date` task_id whose `optional` array runs a further 4
weeks past the last `origin_date` (derived target end dates for the
last round's horizons) -- matching dates anywhere in the whole file
would wrongly admit those as origin dates too.
"""
function load_valid_origin_dates(hub_path)::Set{Date}
    path = joinpath(hub_path, "hub-config", "tasks.json")
    text = read(path, String)
    key = findfirst("\"origin_date\":", text)
    key === nothing && error("no \"origin_date\": key found in $path")
    rest = text[last(key):end]
    open_idx = findfirst('[', rest)
    open_idx === nothing &&
        error("no '[' found after \"origin_date\": in $path")
    close_idx = findfirst(']', rest[open_idx:end])
    close_idx === nothing &&
        error("no ']' closing origin_date's optional array in $path")
    section = rest[open_idx:(open_idx + close_idx - 1)]
    return Set(Date(m.match) for m in eachmatch(r"\d{4}-\d{2}-\d{2}", section))
end
...
function main()
    ...
    if hub_path !== nothing
        valid_origins = load_valid_origin_dates(hub_path)
        n_before = nrow(forecast)
        forecast = forecast[in.(forecast.origin_date, Ref(valid_origins)), :]
        n_dropped = n_before - nrow(forecast)
        if n_dropped > 0
            println("dropped $(n_dropped) row(s) at origin_date(s) " *
                    "outside hub-config/tasks.json's round list")
        end
        write_submission(forecast, hub_path)
        write_metadata(
            MODEL_ID, hub_path;
            team_abbr="nfidd", model_abbr="seasonpool", designated=true,
        )
