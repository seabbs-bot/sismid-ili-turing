...
#
# Combo 6 (PRIORITY, flagged by Sam as the likely front-runner):
# differencing -- an ARIMA(`diff_order`, 1, 0)-style residual, i.e. an
# AR(`diff_order`) model fit to the FIRST DIFFERENCE of the residual and
# integrated back at forecast time (`simulate_diff_paths`), in place of
# combo 1's plain AR-on-the-level residual. Swept over `diff_order` AND
# crossed against pooled-seasonal on/off and backfill on/off, so both
# "differencing vs AR-on-residual" and "with/without the pooled
# seasonal" are directly comparable, not just asserted. The priority
# target is season=on, backfill=on (matching combo 1's design exactly
# except for the differencing).
...
"""
    simulate_diff_paths(dy, coef, resid_sd, order, last_level, horizons,
                         npaths; rng) -> Dict{Int,Vector{Float64}}

ARIMA(`order`, 1, 0)-style forward simulation: `dy` is the
FIRST-DIFFERENCED residual series (`diff(y)`, one shorter than `y`),
already fit by `fit_ar(dy, order)` (an AR(`order`) model on the
differences themselves, so `coef`/`resid_sd` describe the DIFFERENCE
dynamics, not the level). Each simulated path draws a fresh difference
`d_{T+h}` exactly as `simulate_paths` would for `dy`, but then
integrates it by adding to a running level starting at `last_level`
(the last observed, UN-differenced value of `y`), so the values
returned per horizon are back on the same level scale as
`simulate_paths` -- callers can treat the two interchangeably.
"""
function simulate_diff_paths(
    dy::AbstractVector{Float64}, coef::Vector{Float64}, resid_sd::Float64,
    order::Int, last_level::Float64, horizons, npaths::Int;
    rng::Random.AbstractRNG,
)
    hmax = maximum(horizons)
    out = Dict(h => Vector{Float64}(undef, npaths) for h in horizons)
    tail0 = dy[(end - order + 1):end]
    for s in 1:npaths
        tail = copy(tail0)
        level = last_level
        for h in 1:hmax
...
mechanisms is active:
  ...
  - `ar_mode=:discounted`: independent per-location discounted AR
    (`tvar` combo).
  - `ar_mode=:diff`: independent per-location ARIMA(`diff_order`, 1, 0)
    -- AR(`diff_order`) fit to the FIRST DIFFERENCE of the residual,
    integrated back at forecast time (`diff`/`fit_ar`/
    `simulate_diff_paths`; the `diffseason`/`diffraw` combos).
  - `var_order > 0`: joint ridge-VAR(`var_order`) across locations
    (`ridgevar` combo).

`backfill_profile` (if given) is applied via `apply_backfill_correction!`
before deseasonalizing -- used by the `core` combo and, on request, the
differencing combos.
"""
function build_forecast_table(
    ...
    diff_order::Int=6, var_order::Int=0, var_lambda::Float64=1.0,
    model_id::String,
)
    ...
    for season in seasons
        ...
        for split in splits
            ...
            if var_order > 0
                ...
                for (li, loc) in enumerate(LOCATIONS)
                    y = R[:, li]
                    paths = if ar_mode == :diff
                        dy = diff(y)
                        coef, resid_sd = fit_ar(dy, diff_order)
                        simulate_diff_paths(
                            dy, coef, resid_sd, diff_order, y[end],
...
function main()
    ...
    # --- priority combo (Sam): differencing + pooled seasonality + ---
    # --- backfill ("ARIMA(p,1,0)-style" residual on the deseasonalized ---
    # --- series). Swept over diff_order, and crossed against
    # season on/off x backfill on/off so "differencing vs AR-on-residual"
    # and "with/without the pooled seasonal" are both directly visible,
    # not just asserted.
    diff_cells = NamedTuple[]
    for (season_on, prof) in ((true, profile), (false, empty_profile))
        for (backfill_on, bfprof) in (
            (true, backfill_profile), (false, nothing),
        )
            cell_results = NamedTuple[]
            for d in DIFF_ORDERS
                fc = build_forecast_table(
                    VALIDATION_ONLY, versions_full, prof, ones_amp;
                    backfill_profile=bfprof, backfill_window=BF_WINDOW,
                    ar_mode=:diff, diff_order=d,
                    model_id="seasoncombo-diff",
                )
                summ = score_one(fc, truth)
                push!(cell_results, (
                    diff_order=d, mean_wis=summ.mean_wis, sd_wis=summ.sd_wis,
                ))
                println("combo 6 (diff) season=$season_on " *
                        "backfill=$backfill_on diff_order=$d -> " *
                        "mean_wis=$(round(summ.mean_wis; digits=4)) " *
                        "sd_wis=$(round(summ.sd_wis; digits=4))")
            end
            sort!(cell_results; by=r -> r.mean_wis)
            push!(diff_cells, (
                season_on=season_on, backfill_on=backfill_on,
                best=cell_results[1], all=cell_results,
            ))
        end
    end
    # The priority target: season ON, backfill ON.
    diff_target = only(filter(
        c -> c.season_on && c.backfill_on, diff_cells,
    ))
    diff_target_best = diff_target.best
    # Re-run the priority target's best diff_order to get its scored
    # table for the region/time breakdown below.
    diff_target_fc = build_forecast_table(
        VALIDATION_ONLY, versions_full, profile, ones_amp;
        backfill_profile=backfill_profile, backfill_window=BF_WINDOW,
        ar_mode=:diff, diff_order=diff_target_best.diff_order,
        model_id="seasoncombo-diff-target",
    )
    diff_target_scored = score_forecasts(diff_target_fc, truth; scale=:natural)
    ...
    combos = [
        ...
        (name="diff (season+backfill)", mean_wis=diff_target_best.mean_wis,
         sd_wis=diff_target_best.sd_wis,
         detail="diff_order=$(diff_target_best.diff_order), " *
                 "backfill window=$BF_WINDOW ($BF_MODE/per-loc/$BF_STAT)"),
    ]
    ...
    open(joinpath(HERE, "score.txt"), "w") do io
        ...
        println(io)
        println(io, "=== PRIORITY combo 6: differencing " *
                     "(ARIMA(p,1,0)-style residual), season x backfill " *
                     "crossed ===")
        println(io, "diff_order in $DIFF_ORDERS swept in every cell; " *
                     "\"season\"=pooled seasonal shape on/off, " *
                     "\"backfill\"=$BF_WINDOW-week additive/per-loc/median " *
                     "correction on/off")
        for c in diff_cells
            println(io, "season=$(c.season_on) backfill=$(c.backfill_on):")
            for r in c.all
                println(io, "    diff_order=$(r.diff_order) -> " *
                             "mean_wis=$(round(r.mean_wis; digits=4)) " *
                             "sd_wis=$(round(r.sd_wis; digits=4))")
            end
            println(io, "  best: diff_order=$(c.best.diff_order) " *
                         "mean_wis=$(round(c.best.mean_wis; digits=4)) " *
                         "sd_wis=$(round(c.best.sd_wis; digits=4))")
        end
        println(io)
        println(io, "PRIORITY TARGET (season=true, backfill=true, Sam's " *
                     "front-runner): diff_order=" *
                     "$(diff_target_best.diff_order)")
        println(io, "mean_wis=$(round(diff_target_best.mean_wis; digits=4)) " *
                     "sd_wis=$(round(diff_target_best.sd_wis; digits=4))")
        vs_ref_diff = 0.359 - diff_target_best.mean_wis
        vs_pct_diff = 100 * vs_ref_diff / 0.359
        println(io, "vs AR(6)+backfill reference (0.359): " *
                     "$(round(vs_ref_diff; digits=4)) " *
                     "($(round(vs_pct_diff; digits=2))%)")
        vs_core_diff = core_summ.mean_wis - diff_target_best.mean_wis
        vs_pct_core = 100 * vs_core_diff / core_summ.mean_wis
        println(io, "vs combo 1 core (AR-on-residual, no differencing, " *
                     "$(round(core_summ.mean_wis; digits=4))): " *
                     "$(round(vs_core_diff; digits=4)) " *
                     "($(round(vs_pct_core; digits=2))%, " *
                     "$(vs_core_diff > 0 ? "differencing helps" :
                         "differencing HURTS -- AR-on-residual is better"))")
        println(io)
        println(io, "-- priority target: breakdown by location vs core --")
        diff_by_loc = combine(groupby(diff_target_scored, :location),
            :wis => mean => :mean_wis, nrow => :n)
        core_by_loc = combine(groupby(
            score_forecasts(core, truth; scale=:natural), :location,
        ), :wis => mean => :mean_wis)
        merged_dloc = innerjoin(
            diff_by_loc, core_by_loc; on=:location,
            renamecols="_diff" => "_core",
        )
        merged_dloc.improvement = merged_dloc.mean_wis_core .-
                                   merged_dloc.mean_wis_diff
        sort!(merged_dloc, :improvement; rev=true)
        for row in eachrow(merged_dloc)
            println(io, rpad(row.location, 16) *
                         "diff=$(round(row.mean_wis_diff; digits=4)) " *
                         "core=$(round(row.mean_wis_core; digits=4)) " *
                         "improvement=$(round(row.improvement; digits=4))")
        end
        println(io)
        println(io, "-- priority target: breakdown by season vs core --")
        diff_target_scored.season_year = season_year.(
            diff_target_scored.origin_date,
        )
        core_scored_tmp = score_forecasts(core, truth; scale=:natural)
        core_scored_tmp.season_year = season_year.(
            core_scored_tmp.origin_date,
        )
        diff_by_season = combine(groupby(diff_target_scored, :season_year),
            :wis => mean => :mean_wis, nrow => :n)
        core_by_season = combine(groupby(core_scored_tmp, :season_year),
            :wis => mean => :mean_wis)
        merged_dseason = innerjoin(
            diff_by_season, core_by_season; on=:season_year,
            renamecols="_diff" => "_core",
        )
        merged_dseason.improvement = merged_dseason.mean_wis_core .-
                                      merged_dseason.mean_wis_diff
        for row in eachrow(merged_dseason)
